...
class InjectionSightline(BaseModel):
    def __init__(
        self, stars, rvelo, dAVdd=None, injectRealContinuum=True, bins=None, **kwargs
    ):
        super().__init__()
        self.actual_DIBs = np.copy(self.signals)
        self.actual_DIB_errs = np.copy(self.signal_errs)
        self.makeSyntheticDIBs(
            rvelo, dAVdd=dAVdd, injectRealContinuum=injectRealContinuum
        )
    ...
    def get_DIBs(
        ...
        if alternative_data_processing is not None:
            ...
            for i in range(len(self.stars)):
                ...
                signals[i, :], signal_errs[i, :] = (
                    sig[self.self.window],
                    err[self.self.window],
                )
